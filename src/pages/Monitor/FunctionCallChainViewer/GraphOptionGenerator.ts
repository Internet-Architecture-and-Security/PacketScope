...
export const generateGraphOption = (
  data: ChainData | null,
  targetChainIndex: 'all' | number,
  ...
  chartInstanceRef: React.RefObject<any>
) => {
  const rawData = data;
    ...
    nodes.set(functionKey, {
      ...
      label: {
        ...
        formatter: (params: any) =>
          intl.formatMessage(
            { id: 'FunctionCallChainViewer.nodeLabel' },
            { name: params.data.name, count: params.data.value[1], avgDuration: params.data.value[0].toFixed(1) }
          ),
        fontSize: 10,
        fontWeight: '500',
        color: '#FFFFFF',
  ...
  const chartSubtext =
    targetChainIndex === 'all'
      ? intl.formatMessage(
          { id: 'FunctionCallChainViewer.aggregatedGraphSubtext' },
          {
            durationFilterStart: durationFilter[0],
            durationFilterEnd: durationFilter[1],
            nodesCount: nodes.size,
            linksCount: processedLinks.length,
          }
        )
      : intl.formatMessage(
          { id: 'FunctionCallChainViewer.singleGraphSubtext' },
          {
            threadId: rawData[currentChainType]?.[targetChainIndex]?.[0]?.[3],
            nodesCount: nodes.size,
            linksCount: processedLinks.length,
          }
        );

  // Create refresh function for chart
  const createRefreshFunction = async (queryParams: QueryParams | null) => {
    const chart = chartInstanceRef.current;
    if (!chart || !queryParams) return;
    ...
    try {
      ...
      formData.append('srcip', queryParams.srcip);
      formData.append('dstip', queryParams.dstip);
      formData.append('srcport', queryParams.srcport.toString());
      formData.append('dstport', queryParams.dstport.toString());
      formData.append('count', '20000');
      ...
      const updatedData = {
        [receiveChainName]: responseData[0],
        [sendChainName]: responseData[1],
      };

      chart.hideLoading();
      const newOption = generateGraphOption(
        updatedData,
        targetChainIndex,
        durationFilter,
        funcTable,
  ...
  return {
          ...
          return `
            <div style="line-height: 1.6;">
              <div style="font-weight: bold; font-size: 14px; margin-bottom: 8px; color: #2b6cb0;">
                ${intl.formatMessage(
                  { id: 'FunctionCallChainViewer.tooltip.functionTitle' },
                  { category: params.data.category, name: params.data.name }
                )}
              </div>
    ...
    toolbox: {
      ...
      feature: {
        ...
        myrefresh: {
          show: true,
          title: intl.formatMessage({ id: 'FunctionCallChainViewer.refresh' }),
          ...
          type: 'myrefresh',
          onclick: () => createRefreshFunction(queryParams),
        },
      },
    },
    visualMap: [
      {
        ...
        text: [
          intl.formatMessage({ id: 'FunctionCallChainViewer.high' }),
          intl.formatMessage({ id: 'FunctionCallChainViewer.low' }),
          intl.formatMessage({ id: 'FunctionCallChainViewer.receiveDuration' }),
        ],
        textGap: 10,
        left: 'left',
        top: 'middle',
        calculable: true,
        realtime: false,
        inRange: { opacity: [0.8, 1], color: ['#2b6cb000', '#2b6cb0'] },
      ...
      },
      {
        ...
        calculable: true,
        realtime: false,
        inRange: { opacity: [0.9, 1], color: ['#d69e2e00', '#d69e2e'] },
        outOfRange: { opacity: 0 },
        controller: { inRange: { opacity: [0.3, 1] }, outOfRange: { opacity: 0 } },
