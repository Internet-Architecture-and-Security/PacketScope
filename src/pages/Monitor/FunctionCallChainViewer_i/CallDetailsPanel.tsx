...
import { ProcessedCall, FuncTable, SelectedCall } from './types';

interface CallDetailsPanelProps {
  selectedCall: SelectedCall;
  processedData: ProcessedCall[][];
  funcTable: FuncTable;
  onClose: () => void;
}
...
const CallDetailsPanel: React.FC<CallDetailsPanelProps> = ({ selectedCall, processedData, funcTable, onClose }) => {
  const intl = useIntl();
  ...
  return (
    <div className="w-1/4 min-w-[280px] bg-white border-l border-gray-200 flex flex-col">
      <div className="px-2 py-1 bg-gray-50 border-b border-gray-200 flex items-center justify-between">
        ...
        <Button type="text" size="small" icon={<X className="w-4 h-4 text-slate-500" />} onClick={onClose} />
      </div>
