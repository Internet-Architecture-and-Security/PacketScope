...
// U32ToIpv4 converts a 32-bit integer to IPv4 string
// Note: BPF stores IP addresses in little-endian (host byte order), so we need to reverse the bytes
func U32ToIpv4(ip uint32) string {
	return fmt.Sprintf("%d.%d.%d.%d",
		byte(ip),     // 原第4字节 -> 第1字节
		byte(ip>>8),  // 原第3字节 -> 第2字节
		byte(ip>>16), // 原第2字节 -> 第3字节
		byte(ip>>24)) // 原第1字节 -> 第4字节
}
	...
	go func() {
		...
		for {
			...
			eventsChan <- EventToStore{
				...
				ID:      packet.FuncID,
				PID:     uint32(packet.Pid), // 修复：取低32位作为PID，进程ID通常存储在低32位
